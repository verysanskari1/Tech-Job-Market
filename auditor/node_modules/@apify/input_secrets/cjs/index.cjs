...
var OMIT_KEYS = /* @__PURE__ */ new Set([
  "title",
  "description",
  "sectionCaption",
  "sectionDescription",
...
var ENCRYPTED_VALUE_REGEXP = new RegExp(
...
function encryptInputSecretValue({
  value,
  publicKey,
  schema
}) {
  ...
  for (const key of secretsInInputKeys) {
    ...
    if (value && !(ow.isValid(value, ow.string) && ENCRYPTED_VALUE_REGEXP.test(value))) {
      try {
        ...
        throw new Error(
          `The input field "${key}" could not be encrypted. Try updating the field's value in the input editor. Encryption error: ${err}`
        );
      }
    }
  }
