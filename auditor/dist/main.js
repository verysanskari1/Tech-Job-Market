...
const { data: lowConfData, error: lowConfError } = await supabase
    ...
    .limit(halfSize);
if (lowConfError)
    throw new Error(`Failed to fetch low-confidence roles: ${lowConfError.message}`);
// ------------------------------------------------------------------
...
const { data: allClassified, error: allError } = await supabase
    ...
    .select('raw_role_id, category, confidence');
if (allError)
    throw new Error(`Failed to fetch classified roles: ${allError.message}`);
...
const lowConfIds = new Set((lowConfData ?? []).map(r => r.raw_role_id));
const randomSample = shuffle(allClassified ?? [])
    .filter(r => !lowConfIds.has(r.raw_role_id))
    .slice(0, halfSize);
const seen = new Set();
const sampled = [];
for (const r of [...(lowConfData ?? []), ...randomSample]) {
    const id = r.raw_role_id;
    if (!seen.has(id)) {
        seen.add(id);
        sampled.push({ raw_role_id: id, category: r.category, confidence: r.confidence });
    }
}
...
const { data: rawRolesData, error: rawRolesError } = await supabase
    ...
    .in('id', roleIds);
if (rawRolesError)
    throw new Error(`Failed to fetch raw_roles: ${rawRolesError.message}`);
const rawRolesMap = new Map((rawRolesData ?? []).map(r => [
    r.id,
    { title: r.title_raw, department: r.department_raw },
]));
const rolesToAudit = sampled
...
for (let i = 0; i < rolesToAudit.length; i += BATCH_SIZE) {
    ...
    const results = await Promise.all(batch.map(async (role) => {
        try {
            const result = await auditRole(role.original_category, role.original_confidence, role.title, role.department);
            return {
            ...
            log.error(`Audit failed for "${role.title}": ${err.message}`);
            return null;
        }
    }));
...
const avgAuditConfidence = auditRows.length > 0
    ? auditRows.reduce((sum, r) => sum + r.audit_confidence, 0) / auditRows.length
    : 0;
const disputedByTitle = new Map();
